...
// }
  ...
  async loadSchedules(params : {[key:string] : any}){
    ...
    const { doctorShedules, errors } = await GraphQLQueryBuilder.fetchQuery(query, this.graphqlEndpoint);
  ...
  async create(input: { [key: string]: any }, params? : {[key:string] :any}): Promise<Result<any, string>> {
    ...
    const { createDoctorShedule, errors } = await GraphQLQueryBuilder.fetchQuery(mutation, this.graphqlEndpoint);
